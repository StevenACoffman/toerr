...
exactly wrong for a package-level sentinel. A counter captured then would point at
package initialization rather than any failure, and you would pay to record it on
every declaration. `sentinel.New` captures nothing. It has no `Is` method either, so
